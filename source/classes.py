...
class FPL_data:
    def __init__(self, input_df = None):
        ...
        if input_df == None:
            ...
            self.input_data, self.team_dict, self.player_dict = fetch_gameweek_data(self.players, self.teams)
    ...
    def build_svr(self, variable='total_points', type_scaler= 'standard', kernel_type = 'rbf', feature_selection = r_regression):
        (self.svr_regression, self.scaler, self.feature_selector,
         self.train_data_type, self.data_dict) = build_svr(self.ML_input,
                                                           predicted_value=variable,
                                                           scaler_type = type_scaler,
                                                           kernel_type = kernel_type,
                                                           feature_selection = feature_selection)

    def build_knn(self, variable = 'total_points', type_scaler = 'standard', weights = 'uniform', n_neighbors = 5, feature_selection = r_regression):
        (self.knn_regression, self.scaler,
         self.feature_selector,self.train_data_type, self.data_dict) = build_knn(self.ML_input,
                                                                                 predicted_value = variable,
                                                                                 scaler_type= type_scaler,
                                                                                 weights = weights,
    ...
    def build_dtr(self, variable = 'total_points', type_scaler = 'standard', criterion = 'squared_error', feature_selection = r_regression):
        (self.dtr_regression, self.scaler,
         self.feature_selector, self.train_data_type, self.data_dict) = build_dtr(self.ML_input,
                                                                                  predicted_value = variable,
                                                                                  scaler_type = type_scaler,
                                                                                  criterion = criterion,
                                                                                  feature_selection = feature_selection)

    def build_mlp(self, variable = 'total_points', type_scaler = 'standard', max_iter = 500, feature_selection = r_regression):
        (self.mlp_regression, self.scaler,
         self.feature_selector, self.train_data_type, self.data_dict) = build_mlp(self.ML_input,
                                                                                  predicted_value = variable,
                                                                                  scaler_type = type_scaler,
                                                                                  max_iter = max_iter,
                                                                                  feature_selection = feature_selection)
    def predict(self, model):
        ...
        y_test = self.data_dict['y_test']['raw']
